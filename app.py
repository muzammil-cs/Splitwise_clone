...
def dashboard():
    ...
    for e in current_user.paid_expenses.order_by(Expense.created_at.desc()).all():
        ...
        # split_amount = (
        #     float(e.total_amount) / num_participants if num_participants else 0
        # )
    ...
    owes_expenses = []
    for p in current_user.participations.order_by(
        ExpenseParticipant.added_at.desc()
    ).all():
        e = p.expense
    ...
    # Calculate totals
    you_are_owed = sum(
        p["amount"] for e in paid_expenses for p in e["participants"] if not p["paid"]
    )
        ...
        if num_participants > 0:
            ...
            for user_id in participants_ids:
                participant = ExpenseParticipant(
                    user_id=int(user_id), expense_id=expense.id, amount=split_amount
                )
...
def update_expense(expense_id):
    ...
    if request.method == "POST":
        ...
        num_participants = len(participants_ids)
        split_amount = (
            expense.total_amount / num_participants if num_participants else 0
        )
